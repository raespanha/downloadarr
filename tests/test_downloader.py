...
async def test_parallel_exact_hash(server, tmp_path, connections):
    ...
    result = await Downloader(DownloadConfig(connections=connections)).download(
...
async def test_truncated_chunk_preserves_partial_and_no_final(server, tmp_path):
    ...
    output = tmp_path / "out"
    with pytest.raises(ProtocolError):
        await Downloader(DownloadConfig(connections=4)).download(await provider(url), output)
...
async def test_manifest_resume(server, tmp_path):
    url = await server(range_handler())
    output = tmp_path / "out"
    chunks = Downloader(DownloadConfig(connections=4))._chunks(len(DATA), True)
    first = chunks[0]
...
async def test_manifest_is_checkpointed_in_batches(server, tmp_path, monkeypatch):
    ...
    config = DownloadConfig(connections=4, block_size=1024, checkpoint_bytes=20_000,
...
async def test_concurrent_expiry_refreshes_once(server, tmp_path):
    ...
    async def urls(refresh):
        ...
        refreshes += int(refresh)
        return url
    await Downloader(DownloadConfig(connections=4, backoff_base=0)).download(urls, tmp_path / "out")
    assert refreshes == 1
